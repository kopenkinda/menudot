...
1. The app uses your selected Launch state, which defaults to Stopped.
...
4. Press **Start**. Click its centered menu bar dot to switch bars.
...
**Restore all icons** stops the session without forgetting choices. Escape in settings does the same. Launch state chooses whether each launch starts switching or stays stopped. Quitting or putting the Mac to sleep also releases the visibility session.
...
The app reads the macOS menu bar host's Accessibility item tree. It does not use the running-process list as the icon inventory. Previously detected icons remain editable when hidden or temporarily absent. Earlier saved choices that have never been verified as menu bar icons are omitted from settings.
